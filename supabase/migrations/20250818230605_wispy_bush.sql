-- =========================================================
-- PawPilot HQ — Bucket Standardization Runbook
-- Objective: Standardize storage buckets to underscores in code
-- Keep legacy hyphen buckets readable (read-only) so nothing breaks
-- Enforce path-based ownership (<uid>/filename) for writes
-- =========================================================
...
begin
  ...
  loop
    -- public read
    if not exists (select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname = initcap(replace(b,'_',' ')) || ': public read') then
    ...
    if not exists (select 1 from pg_policies where policyname = initcap(replace(b,'_',' ')) || ': auth insert (own folder)') then
    ...
    -- owner update
    if not exists (select 1 from pg_policies where policyname = initcap(replace(b,'_',' ')) || ': owner update') then
    ...
    -- owner delete
    if not exists (select 1 from pg_policies where policyname = initcap(replace(b,'_',' ')) || ': owner delete') then
...
do $$
begin
  if not exists (select 1 from pg_policies where policyname='Health attachments: auth select (own)') then
  ...
  if not exists (select 1 from pg_policies where policyname='Health attachments: auth insert (own folder)') then
  ...
  if not exists (select 1 from pg_policies where policyname='Health attachments: owner update') then
  ...
  if not exists (select 1 from pg_policies where policyname='Health attachments: owner delete') then
...
do $$
begin
  if not exists (select 1 from pg_policies where policyname='Group avatars (legacy): public read') then
    create policy "Group avatars (legacy): public read" on storage.objects for select using (bucket_id='group-avatars');
  end if;
  if not exists (select 1 from pg_policies where policyname='Album photos (legacy): public read') then
    create policy "Album photos (legacy): public read" on storage.objects for select using (bucket_id='album-photos');
  end if;
  if not exists (select 1 from pg_policies where policyname='Lost & found (legacy): public read') then
    create policy "Lost & found (legacy): public read" on storage.objects for select using (bucket_id='lost-found-photos');
  end if;
  if not exists (select 1 from pg_policies where policyname='Reel videos (legacy): public read') then
    create policy "Reel videos (legacy): public read" on storage.objects for select using (bucket_id='reel-videos');
  end if;
  if not exists (select 1 from pg_policies where policyname='Cause images (legacy): public read') then
...
select
  ...
  end as bucket, policyname, cmd
from p
order by bucket, policyname, cmd;